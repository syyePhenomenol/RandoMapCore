    ...
    : PlacementProgressHint(RandoPlacement)
{
    ...
    internal override string GetTextFragment()
    {
        ...
        if (RandoMapCoreMod.GS.ProgressHint is ProgressHintSetting.Location)
        {
            text += $"\n{"at".L()} {RandoPlacement.Location.Name.LC()}";
        }

        if (RandoMapCoreMod.GS.ProgressHint is ProgressHintSetting.Location or ProgressHintSetting.Room)
        {
            text += $"\n{"in".L()} ";
    ...
    internal override void DoPanning()
    {
        ...
        switch (RandoMapCoreMod.GS.ProgressHint)
        {
    ...
    {
        if (!RmcPinManager.Pins.TryGetValue(location, out var pin))
        {
            return false;
        }
    ...
    : PlacementProgressHint(RandoPlacement)
{
    internal override bool IsPlacementObtained()
    {
        return RandoMapCoreMod.Data.VisitedTransitions.ContainsKey(RandoPlacement.Location.Name);
    }
    ...
    internal override string GetTextFragment()
    {
        ...
        if (RandoMapCoreMod.GS.ProgressHint is ProgressHintSetting.Room)
        {
    ...
    private static bool TryPanToScene(string scene)
    {
        ...
        if (RmcRoomManager.RoomTexts.TryGetValue(scene, out var rt))
        {
